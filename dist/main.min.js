...
;(function(){
    ...
    $(document).click(function(e) {
        const button = $('.modal__register');
        if (e.target == button[0]){
            const target = e.target;
            const modalForm =target.closest('.modal__form--wrap');

            $(modalForm).children('.modal__enter').toggleClass('modal__hidden');
            $(modalForm).children('.modal__regist').toggleClass('modal__hidden');

            if ($('.modal__enter').hasClass('modal__hidden')) {
                $(target).text('Зарегистрироваться');
                $('.modal__title').text('Вход');
                $(target).closest('.modal').scrollTop(0);
                $(target).closest('.modal').css({'overflow' : 'hidden'});
            }else{
                $(target).text('Я уже зарегистрирован');
                $('.modal__title').text('Регистрация');
                $(target).closest('.modal').css({'overflow' : 'scroll'});
            }
        }
    })
})();;$(".dropdown__btn").hover(
    function(){
      $(this).parent().find('.dropdown').toggleClass("dropdown__opened")
    }
);

$(".dropdown").hover(
    function(){
      $(this).toggleClass("dropdown__opened")
    }
);;(function(){
    $('.menu--switcher').on('click', function(){
        $('.menu--mb').toggleClass('menu--mb--opened');
    });
})();
;(function(){
    $('.nav-pannel--lc').click(function() {
        // Same initializing per every showing
        $('#modal').plainModal('open', {duration: 500});
    });

    $('.nav-pannel--bs').click(function() {
        // Same initializing per every showing
        $('#modalBasket').plainModal('open', {duration: 500});
    });
})();;(function(){
  ...
  const swiper = new Swiper('.swiper-container', {
    ...
    // And if we need scrollbar
      ...
      $('ul.tabs__caption').on('click', 'li:not(.active)', function() {
        ...
        $(this)
          .addClass('tabs__button--active').siblings().removeClass('tabs__button--active')
          .closest('div.tabs').find('div.tabs__content').removeClass('tabs__content--active').eq($(this).index()).addClass('tabs__content--active');
      });
      
      // табы в чате

      $('ul.chat__list').on('click', 'li:not(.active)', function() {
                  
        $(this)
          .addClass('chat__item--active').siblings().removeClass('chat__item--active')
          .closest('div.tabs').find('div.chat__content').removeClass('chat__content--active').eq($(this).index()).addClass('chat__content--active');
          // здесь идет реализация блока с чатом на мобилках
          if($(window).width() < 768){
            $('.chat__left').addClass('chat__left--none');
            $('.chat__right').addClass('chat__right--active');
            None();

            // здесь закрывается само окно чата
            $('.chat__back').on('click', function(){
              $('.chat__left').removeClass('chat__left--none');
              $('.chat__right').removeClass('chat__right--active');
              $('.chat').removeClass('chat__bg');
              $('.nav__small').removeClass('none');
              $('.order__message').removeClass('none');
              $('.search.chat__search').removeClass('none');
              $('.chat__item').removeClass('chat__item--active');
            });
          }
      ...
      if($(window).width() < 768){
        $('.chat__item').removeClass('chat__item--active');
        // if($('.chat__item').hasClass('chat__item--active')){
        //   $('.chat__left').addClass('chat__left--none');
        //   $('.chat__right').addClass('chat__right--active');
        // }
      }
